// Next.js API route to proxy /users/payment/list requests to avoid CORS issues
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(`${userApiUrl}/payment/list?status=paid`, {
      method: "GET",
      headers,
    });
    ...
    console.log("=== USER PAYMENTS PROXY SUCCESS ===");
    console.log("Response status:", response.status);
    return res.status(response.status).json(responseData);
  } catch (error) {
    console.error("User payments proxy error:", error);
